...
5. Verifies `C:\Program Files (x86)\Steam\steamapps\common\Starfield\dxgi.dll` exists and has >= 3 `CreateDXGIFactory` exports
...
```powershell
# RelWithDebInfo build (has PDB)
powershell -ExecutionPolicy Bypass -File .claude\skills\run-starfield2vr\smoke.ps1 `
    -Preset release-msvc -ConfigPreset build-release-msvc
