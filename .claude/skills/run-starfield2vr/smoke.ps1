# smoke.ps1 -- build starfield2vr (sf2vr) and verify the output DLL
# Run from repo root:
...
# 4. Build (remove stale dxgi.lib to avoid LNK1149)
Remove-Item (Join-Path $buildDir "dxgi.lib") -Force -ErrorAction SilentlyContinue
Remove-Item (Join-Path $buildDir "dxgi.dll") -Force -ErrorAction SilentlyContinue
...
# 5. Verify DLL in Steam install dir (POST_BUILD copies it there)
Write-Host ""
Write-Host "=== Verify DLL ==="
$dll = "C:\Program Files (x86)\Steam\steamapps\common\Starfield\dxgi.dll"
...
Write-Host "DLL deployed to: $dll"
